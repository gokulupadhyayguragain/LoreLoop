...
export default function HomePage() {
  ...
  return <>
    ...
    <section className="section"><div className="container"><SectionHeading eyebrow="The world so far" title="Latest evolution." copy="Every entry is born from the canon that came before it. This archive is the visible edge of an autonomous process." link={{ href: "/timeline", label: "View timeline" }} />{latest ? <div className="feature-lore"><Artwork lore={latest} large /><div className="feature-copy"><div className="meta-row"><span>{formatEntityType(latest.entityType)}</span><span>{latest.region || "Uncharted region"}</span><span>{latest.era || "The Third Age"}</span></div><h3>{latest.title}</h3><p>{latest.summary}</p><div className="feature-foot"><span className="autonomous-badge">Generation {String(latest.generationNumber).padStart(3, "0")} · Autonomous</span><span>{formatDate(latest.generatedAt)}</span></div><Link href={`/lore?id=${encodeURIComponent(latest.id)}`} className="button-link" style={{ marginTop: 25, alignSelf: "flex-start" }}>Read the entry →</Link></div></div> : error ? <ErrorState message={error} /> : <EmptyState />}</div></section>
