...
export default function LoreDetailPage() {
  ...
  const [lore, setLore] = useState<LoreEntity | null>();
  const [error, setError] = useState<string>();

  useEffect(() => {
    const value = new URLSearchParams(window.location.search).get("id");
    setId(value ?? undefined);
    if (value) void api.loreById(value).then((result) => { setLore(result.data); setError(result.error?.message); });
  }, []);

  if (!id) return <div className="container"><div style={{ padding: "100px 0" }}><EmptyState title="Choose a page from the archive." message="This lore detail needs an identifier from the living world." /></div></div>;
